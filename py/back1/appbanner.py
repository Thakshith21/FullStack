...
if len(sys.argv) != 13:
    print("Usage: python script1.py title subtitle ... reg")
    sys.exit(1)
...
base_path = "/var/www/html/thakshith/templatetool/"

# Open base image
base = Image.open(f"{base_path}inputimg/appbanner.png").convert("RGBA")

# Load overlay images
logo = Image.open(f"{base_path}/examlogos/{logo}-130.png").convert("RGBA")
himanshu = Image.open(f"{base_path}/facimg/{facimg1}-170.png").convert("RGBA")
sanyam = Image.open(f"{base_path}/facimg/{facimg2}-170.png").convert("RGBA")
...
def draw_wrapped_text_with_resize(draw, position, text, font, fill,
    ...
    while True:
        line_height = draw.textsize("A", font=current_font)[1]
...
# Save final output
base.save(f"{base_path}output/appbanner.png")
